...
fn main() {
    ...
    let mods = fs::read_dir(&crates_dir)
        ...
        .filter_map(|v| {
            ...
            if name.starts_with(".") || name == "target" || name == "wasm32-unknown-unknown" || name == "release" || name == "precompiled" {
                return None;
            }
        ...
        .map(|v| -> Result<String> {
            ...
            let f: Vec<u8> = fs::read(
                &Path::new(&wasm_str)
                    ...
                    .join("release")
                    .join(format!("alkane_{}.wasm", subbed)),
            )?;
            let compressed: Vec<u8> = compress(f.clone())?;
            fs::write(&Path::new(&wasm_str).join("wasm32-unknown-unknown").join("release").join(format!("alkane_{}.wasm.gz", subbed)), &compressed)?;
