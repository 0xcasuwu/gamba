...
fn create_gamba_ecosystem_setup() -> Result<(AlkaneId, AlkaneId, OutPoint)> {
    ...
    Ok((factory_contract_id, free_mint_contract_id, mint_outpoint))
}
...
#[wasm_bindgen_test]
fn test_gamba_deposit_redemption() -> Result<()> {
    let (factory_id, _free_mint_id, mint_outpoint) = create_gamba_ecosystem_setup()?;

    // Deposit
    let deposit_block = perform_deposit_with_traces(&factory_id, &mint_outpoint, 1000, 10)?;
    let coupon_outpoint = OutPoint { txid: deposit_block.txdata[0].compute_txid(), vout: 0 };

    // Redemption
    perform_redemption_with_traces(&factory_id, &coupon_outpoint, 12)?;
